...
async function exec() {
  ...
  if(!fs.existsSync(`${__dirname}/../../sequelize.json`)){
    fs.writeFileSync(`${__dirname}/../../sequelize.json`, JSON.stringify([]))
  }

  const sequelizeData = require('../../sequelize.json')
  ...
  fs.writeFileSync(`${__dirname}/../../sequelize.json`, JSON.stringify(dataUpdated))
